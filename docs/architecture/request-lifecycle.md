...
聊天流内的业务响应会进一步分类。明确的永久错误不重试；业务码 500 的 message
不被当作可靠的瞬态信号，最多重放一次；其他可重试业务码使用配置的预算。这个限制
不改变网络错误、HTTP 状态或 read timeout 的既有预算。

认证失败最多触发一次 token 刷新。GLM、Qwen 和 Kimi 的上游流若连续 90 秒没有
新字节，会被视为停滞并最多重试一次。DeepSeek V4 Flash/Pro 使用 60 秒 read timeout
和最多两次停滞重试；三次尝试的名义静默等待上限仍是 180 秒。read timeout 衡量相邻
上游字节间隔，不是整轮推理时限。所有重试仍受“尚未向客户端发送数据”的约束；已有
增量时只返回流内错误，不从头重放。
