...
如果模型仍返回与最近已完成动作同名、同参数的 action，代理不会立即把它交给
客户端，而是要求模型重新检查最新结果并选择下一步。这个确认过程与其他 Kimi 格式
修复共用最多三次上游尝试；若最后一次仍坚持相同 action，代理会将它放行，避免阻断
轮询或明确需要的幂等重试。因此，这层保护可以减少意外重复，不能替代工具自身的
幂等设计。

## 流式边界

Kimi 首轮工具请求以及没有最近已完成动作的请求，会尽快转发 reasoning 增量。
`<k3_action>`、`<k3_final>` 及其正文仍要等信封闭合并验证成功后再交付，防止客户端
收到半截 JSON 或协议标签。

带最近完成动作的续轮需要额外防止模型重复调用：代理按上游尝试暂存这一轮的
reasoning 和 action。若该尝试因重复动作被丢弃，对应 reasoning 也不会交给客户端；
最终接受尝试的 reasoning 会先于 action 按原顺序发送。这是 Kimi 受限桥接路径的
正确性边界，不影响其他模型或普通 Kimi 首轮的实时转发。
